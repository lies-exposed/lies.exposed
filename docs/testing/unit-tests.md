...
it("should work", async () => {
  const [data] = fc.sample(SomeArb(), 1);
  mockTERightOnce(mockCtx.db.find, () => expected);
  const result = await pipe(fn(data)(mockCtx), throwTE);
  expect(result).toMatchObject(expected);  // ✅ Specific assertion
});
```

**Critical**: Always use `throwTE`. Never use `expect(x).not.toBe(y)`.

</details>

---

Patterns and best practices for unit testing (`.spec.ts`, `.test.ts` files).
...
- [Test Utilities](./test-utils.md) - Mocking patterns and helpers
- [@liexp/test Package](../packages/test.md) - Arbitraries reference
