...
```typescript
// Mock TaskEither success
mockTERightOnce(mockFn, () => returnValue);

// Mock TaskEither error
mockTELeftOnce(mockFn, () => new Error("message"));

// Mock Option
mockOptionSome(mockFn, value);
mockOptionNone(mockFn);

// Create context
const ctx = mockedContext({ db: mock(), logger: mock() });

// Create user + login
const user = await saveUser(Test.ctx, ["scope:read"]);
const { authorization } = await loginUser(Test)(user);
```

**Use**: `vitest-mock-extended`, `throwTE`, arbitraries from `@liexp/test`.

</details>

---

Patterns and helpers for test utilities, mocks, and factories.
