...
export interface HumanPopulationGrowthGraphAccessibleProps {
  showPoints: boolean;
}
...
/**
 * HumanPopulationGrowthGraph with WCAG 2.1 AA accessibility support
 *
 * This component adds:
 ...
 * - Focus indicators
 */
export const HumanPopulationGrowthGraphAccessible: React.FC<
  HumanPopulationGrowthGraphAccessibleProps
> = ({ showPoints }) => {
  const filteredData = data;
  const populationNumbers = filteredData.map((d) => d.population);
  const description = generateDataDescription(
    populationNumbers,
    "World population",
  );

  const dataTableHTML = createAccessibleDataTable(filteredData, [
    {
      name: "Year",
      accessor: (d: (typeof filteredData)[0]) => d.year.toString(),
    },
    {
      name: "Population (Millions)",
      accessor: (d: (typeof filteredData)[0]) => d.population.toString(),
    },
  ]);
  ...
  return (
    <GraphA11yWrapper
      ...
      title="Human Population Growth 1000 BCE to 2015 CE"
      description={description}
      dataTable={<div dangerouslySetInnerHTML={{ __html: dataTableHTML }} />}
    >
