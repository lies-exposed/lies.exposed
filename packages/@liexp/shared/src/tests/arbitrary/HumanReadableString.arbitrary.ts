...
function generateName(joinChar: string): string {
  const name = `${capFirst(
    name1[getRandomInt(0, name1.length)]
  )}${joinChar}${capFirst(name2[getRandomInt(0, name2.length)])}`;
  return name;
}

interface HumanReadableStringArbArgs {
  joinChar?: string;
}

export const HumanReadableStringArb = (
  args?: HumanReadableStringArbArgs
): fc.Arbitrary<string> => {
