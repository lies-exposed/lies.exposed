...
export const getOrphanMediaFlow = <
...
>(): ReaderTaskEither<
  C,
  FSError | SpaceError | DBError,
  GetOrphanMediaFlowOutput
> => {
  return pipe(
    fp.RTE.ask<C>(),
    fp.RTE.map((ctx) =>
            ...
            (objects) => (ctx) =>
              pipe(
                ...
                fp.A.traverse(fp.TE.ApplicativeSeq)(
                  (e): TaskEither<DBError, ObjectMediaPair> => {
                    if (e.Key) {
                      ...
                      return pipe(
                        ...
                        fp.O.getOrElse(() =>
                          fp.TE.right<DBError, Option<ItemObject>>(fp.O.none),
                        ),
