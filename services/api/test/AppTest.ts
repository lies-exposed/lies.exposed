      ...
      dbOverride
        ...
        : pipe(
            ...
            TE.tap((source) => {
              ...
              console.log(
                `🔍 [APPTEST] DataSource created for ctx.db, marked as __usedByCtxDb`,
              );
...
const initAppTest = async (ctx: ServerContext): Promise<AppTest> => {
...
export const GetAppTest = async (
  ...
  appTestLogger.info.log(
    "Getting app test for worker ID: %s (dbOverride: %s)",
    testFileId,
    !!dbOverride,
  );
  ...
  if (dbOverride) {
    appTestLogger.debug.log(
      "Creating context with transactional DB for worker: %s",
      testFileId,
    );
