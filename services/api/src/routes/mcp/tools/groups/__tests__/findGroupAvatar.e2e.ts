...
import { findGroupAvatarToolTask } from "../findGroupAvatar.tool.js";

describe("MCP FIND_GROUP_AVATAR Tool", () => {
  let Test: AppTest;
  ...
  test("Should return error when group not found on Wikipedia", async () => {
    ...
    const result = await pipe(
      findGroupAvatarToolTask({
        name: "NonexistentGroupXYZ123456",
  ...
  test("Should handle Wikipedia search errors gracefully", async () => {
    ...
    const result = await pipe(
      findGroupAvatarToolTask({
        name: "test with very long name " + "a".repeat(100),
        preferHighRes: false,
      }),
      throwRTE(Test.ctx),
    ).catch((error) => error);
