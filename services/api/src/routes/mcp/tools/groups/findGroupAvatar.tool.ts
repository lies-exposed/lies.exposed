...
/**
 * Find and download group avatar from Wikipedia
 *
 * This tool searches Wikipedia for a group/organization by name, retrieves their profile image,
 * downloads and uploads it to storage, then returns the media ID for use with createGroup.
 *
 * Usage flow:
 * 1. Call findGroupAvatar with group name
 * 2. Tool returns media ID and avatar URL
 * 3. Use the media ID in createGroup config.avatar parameter
 */
export const FindGroupAvatarInputSchema = Schema.Struct({
  name: Schema.String.annotations({
    description:
      "Name of the group/organization to search for on Wikipedia (e.g., 'United Nations')",
  }),
...
export type FindGroupAvatarInputSchema = typeof FindGroupAvatarInputSchema.Type;

export const findGroupAvatarToolTask = ({
  name,
  preferHighRes,
  ...
  return pipe(
    ...
    fp.RTE.chain((ctx) => {
      ...
      const searchWikipediaForAvatar = async (): Promise<URL> => {
        try {
          // Step 1: Search Wikipedia for the group
          const searchResults = await ctx.wp.search(name)();
          if (searchResults._tag === "Left") {
            throw new Error(
              `No Wikipedia results found for "${name}". Try a more complete name.`,
            );
          }
      ...
      return pipe(
        ...
        fp.RTE.chain((url) =>
          uploadMediaFromURLFlow({
            ...
            label: `${name} Avatar`,
            description: `Avatar/profile image for ${name} from Wikipedia`,
          }),
        ),
      );
    }),
