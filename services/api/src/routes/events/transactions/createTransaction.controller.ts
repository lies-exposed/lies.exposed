  ...
  AddEndpoint(r)(
    ...
    ({
      ...
      ctx.logger.debug.log(
        "Create transaction with payload %O",
        payload,
      );

      const fetchOwnersTask = sequenceS(TE.ApplicativePar)({
        from:  ctx.db.findOneOrFail(payload.from.type === 'Actor' ? ActorEntity : GroupEntity, {
              where: { id: payload.from.id },
            }),
        to: ctx.db.findOneOrFail(payload.to.type === 'Actor' ? ActorEntity : GroupEntity, {
          where: { id: payload.to.id },
        }),
      });
