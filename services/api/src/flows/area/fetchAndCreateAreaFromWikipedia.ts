...
export const fetchAndCreateAreaFromWikipedia = (
  title: string,
  wp: WikiProviders,
): TEReader<{ area: AreaEntity; media: MediaEntity[] }> => {
  return pipe(
    fp.RTE.Do,
    fp.RTE.bind("wikipedia", () =>
      pipe(
        fp.RTE.asks<ServerContext, WikipediaProvider, ControllerError>(
          getWikiProvider(wp),
        ),
        fp.RTE.chainTaskEitherK((wp) =>
          pipe(fetchFromWikipedia(title)(wp), fp.TE.mapLeft(toControllerError)),
        ),
      ),
    ),
        ...
        fp.RTE.chain((a) => {
          const saveAreaTask = (
            ...
            fp.O.isSome(a)
              ...
              : pipe(
                  fetchCoordinates(areaData.label)<ServerContext>,
                  fp.RTE.map((geo) => ({
                    ...areaData,
                    ...pipe(
                      geo,
