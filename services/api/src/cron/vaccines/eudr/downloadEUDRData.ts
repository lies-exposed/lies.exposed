...
// const downloadFileName = path.resolve(
//   DATA_DIR_PATH,
//   "Run Line Listing Report.csv"
// );

const Female = t.literal("female");
const Male = t.literal("male");
const NotSpecified = t.literal("not-specified");
const Sex = t.union([Female, Male, NotSpecified], "Sex");
type Sex = t.TypeOf<typeof Sex>;

const ZeroToOneMonth = t.literal("0-1-months");
const TwoMonthsToTwoYears = t.literal("2-months-2-years");
const ThreeToTwelveYears = t.literal("3-12-years");
const TwelveToSixteenYears = t.literal("12-17-years");
const EighteenToSixtyFourYears = t.literal("18-64-years");
const SixtyFiveToEightyfiveYears = t.literal("65-85-years");
const MoreThanEightyFiveYears = t.literal("more-than-85-years");
...
type AgeGroup = t.TypeOf<typeof AgeGroup>;
...
const getFilePath = (
  manufacturer: string,
  { year, ageGroup, gender }: ValuesIndexes
): string => {
  ...
  (
    ...
    return TE.tryCatch(async () => {
      ...
      const newDownloadPath = path.resolve(
        opts.downloadPath,
        getFilePath(manufacturer, indexes)
      );
      ...
      TE.chain((page) => {
        return TE.tryCatch(async () => {
          ...
          const dropdownArrowClassName =
            "div#idComponentLevelTabbarOuterDiv > div > table > tbody > tr > td.secondaryTabBarSpacer.obipsTabBarToolbarCell.DashTabsToolbarCell";
          ...
          const alreadyDownloadedCSVs = fs.readdirSync(opts.downloadPath);
                ...
                A.flatten(
                  yearOptionIndexes.map((y) =>
                    genderOptionIndexes.map((g) => {
                      ...
                      const fileAlreadyDownloaded =
                        alreadyDownloadedCSVs.includes(
                          getFilePath(manufacturer, opts)
                        );
...
export const runDownload = (): TE.TaskEither<Error, void> => {
  ...
  const dateDownloadPath = path.resolve(DATA_DIR_PATH, nowDate, "import");
