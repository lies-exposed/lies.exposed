import { Actor, Common, Group } from "@econnessione/shared/lib/io/http";
...
export const isByActor = (actor: Actor.ActorFrontmatter) => (
  by: Common.ByGroupOrActor
): boolean => by.type === "Actor" && by.actor === actor.id;

export const isByGroup = (group: Group.GroupFrontmatter) => (
  by: Common.ByGroupOrActor
): boolean => by.type === "Group" && by.group === group.id;
